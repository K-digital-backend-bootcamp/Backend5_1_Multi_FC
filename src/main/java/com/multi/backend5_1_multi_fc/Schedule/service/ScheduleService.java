...
public class ScheduleService {
    ...
    public void updatePersonal(String username, Long scheduleId, ScheduleDto.PersonalUpdateReq req) {
        ...
        if (!ok) {
            throw new ScheduleException("일정이 없거나 수정 권한이 없습니다.");
        }
    }
    ...
    public void deletePersonal(String username, Long scheduleId) {
        ...
        if (!ok) {
            throw new ScheduleException("일정이 없거나 삭제 권한이 없습니다.");
        }
    }
    ...
    // ✅ 특정 "월" 전체 일정
    // ==========================
    public List<ScheduleDto.DayItem> allByMonth(String username, YearMonth ym) {
        Long userId = userIdOf(username);

        LocalDate start = ym.atDay(1);
        LocalDate end   = ym.atEndOfMonth();

        List<ScheduleDto.DayItem> result = new ArrayList<>();

        // repo에 "월 단위 조회" 전용 메서드가 없다면
        // 날짜를 하루씩 돌면서 기존 allByDate용 쿼리를 재사용
        for (LocalDate d = start; !d.isAfter(end); d = d.plusDays(1)) {
            result.addAll(repo.findAllByDate(userId, d));
        }
